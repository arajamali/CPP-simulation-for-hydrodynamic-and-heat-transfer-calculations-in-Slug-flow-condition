...
SFConstWallTemp::SFConstWallTemp(double Length, double SuperVelocityGas, double SuperVelocityLiquid, double LiqHoldUp, double TempWall, double TempInlet, double Pressure)
{
	L = Length, TW = TempWall; TLI = TempInlet; VSG = SuperVelocityGas; VSL = SuperVelocityLiquid; P = Pressure; h = LiqHoldUp;
}
...
double SFConstWallTemp::Liquid::Nu(double T, SFConstWallTemp &container) {
	...
	FluidProperties Properties(T, P); // Temp and pressure are needed for properties class
...
double SFConstWallTemp::Liquid::TempProfile(SFConstWallTemp &container) {
	...
	for (int i = 0; i < 200; i++) {
		...
		while (Eps > 10E-12) {
			...
			F = hAverage * SL*X*((TW - T) - (TW - TLO)) / log((TW - T) / (TW - TLO)) - W * CP*(TLO - T);
			Eps = F;
			Fdx = hAverage * SL*X*((TW - T) - (TW - (TLO + dx))) / log((TW - T) / (TW - (TLO + dx))) - W * CP*((TLO + dx) - T);
			Fprime = (Fdx - F) / dx;
			TLO = TLO - F / Fprime;
		}
		T = TLO;
...
double SFConstWallTemp::Gas::TempProfile(SFConstWallTemp &container) {
	...
	for (int i = 0; i < 200; i++) {
		...
		while (Eps > 10E-12) {
			...
			F = hAverage * SG*X*((TW - T) - (TW - TLO)) / log((TW - T) / (TW - TLO)) - W * CP*(TLO - T);
			Eps = F;
			Fdx = hAverage * SG*X*((TW - T) - (TW - (TLO + dx))) / log((TW - T) / (TW - (TLO + dx))) - W * CP*((TLO + dx) - T);
			Fprime = (Fdx - F) / dx;
			TLO = TLO - F / Fprime;
		}
		T = TLO;
...
SFConstHeatFlux::SFConstHeatFlux(double Length, double SuperVelocityGas, double SuperVelocityLiquid, double LiqHoldUp, double InitialTempWall, double TempInlet, double Pressure)
{
	L = Length, TW = InitialTempWall; TLI = TempInlet; VSG = SuperVelocityGas; VSL = SuperVelocityLiquid; P = Pressure; h = LiqHoldUp;
}
...
double SFConstHeatFlux::Liquid::TempProfile(SFConstHeatFlux &container) {
	...
	PipeGeometry PipeGeometry;
	ID = PipeGeometry.IDPipe;
	double ConstTwTo = TWI - TLI;//This is one of the assumptions of constant heat flux approach
	double X = L / 200.0;
	double hAverage;
	double NuL;
	double T = TLI;
	...
	for (int i = 0; i < 200; i++) {
		...
		T = T + hAverage * SL*X*ConstTwTo / (W*CP); // Plus sign here, I am not so sure!!
		TW = T + ConstTwTo;
		cout << T << "	" << TW << endl;
...
double SFConstHeatFlux::Gas::TempProfile(SFConstHeatFlux &container) {
	...
	for (int i = 0; i < 200; i++) {
		...
		T = T + hAverage * SG*X*ConstTwTo / (W*CP); // Plus sign here, I am not so sure!!
		TW = T + ConstTwTo;
		cout << T << "	" << TW << endl;
