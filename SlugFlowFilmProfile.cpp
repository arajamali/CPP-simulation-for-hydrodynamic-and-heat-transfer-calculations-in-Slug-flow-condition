...
namespace MultiphaseTemperature {
	...
	FilmProfile::FilmProfile(std::vector<double>& FilmProfileInputs) {
		...
		//HLLS = 1 - 0.058*(2 * pow(0.4*IT / ((RhoL - RhoG)*9.8), 0.5)*pow(RhoL / IT, 0.6)*pow(2.0/ID* 0.007*pow(VSG+VSL,3), 0.4) - 0.725, 2);
	...
	double FilmProfile::FannningFrictionFactor(double Rho, double V, double ID, double Mu, double Roughness) {
	...
	double FilmProfile::hFdZ(double hF) {
		...
		//std::cout<< (RhoL - RhoG)*g <<" " << (RhoL*VF*(VTb - VLLS)*HLLS*dHLTBdhF / HL / HL) << " " << (RhoG*VG*(VTb - VGLS)*(1 - HLLS)*dHLTBdhF / (1 - HL) / (1 - HL)) << std::endl;
		...
		while (HLTBComparison > EpsilonZh)
		{
			...
			for (zLoop = 0; zLoop < 4000; zLoop++) {
				...
				for (int RKFLoop = 1; RKFLoop < 1000; RKFLoop++) {
					...
					//Cal KK4
					xx = XxAtZ + h * (1932.0 / 2197.0*KK[0] - 7200.0 / 2197.0*KK[1] + 7296.0 / 2197.0*KK[2]);
					F = hFdZ(xx);
					KK[3] = F;

					//Cal KK5
					xx = XxAtZ + h * (439.0 / 216.0*KK[0] - 8.0*KK[1] + 3680.0 / 513.0*KK[2] - 845.0 / 4104.0*KK[3]);
					F = hFdZ(xx);
					KK[4] = F;

					//Cal KK6
					xx = XxAtZ + h * (-8.0 / 27.0*KK[0] + 2.0*KK[1] - 3544.0 / 2565.0*KK[2] + 1859.0 / 4104.0*KK[3] - 11.0 / 40.0*KK[4]);
					F = hFdZ(xx);
					KK[5] = F;

					ERK4 = abs(h*(KK[0] / 360.0 - 128.0 / 4275.0*KK[2] - 2197.0 / 75240.0*KK[3] + KK[4] / 50.0 + 2.0 / 55.0*KK[5]));
					ERKFVal = ERK4;
					if (ERKFVal != 0) {
				...
				//if (Con == 1) {
					//std::cout << z << "	" << hF;
				//}
				//To show each hF at each axial location
				//cout << z << "	" << hF << endl;
				...
				for (int jj = 1; jj < zLoop +2; jj++)
				{
					double HLTBNew = (1 / Pi)*(Pi - acos(2 * (hFArray[jj]) / ID - 1) + (2 * (hFArray[jj]) / ID - 1)*pow(1 - pow(2 * (hFArray[jj]) / ID - 1, 2), 0.5));
					double HLTBPast = (1 / Pi)*(Pi - acos(2 * (hFArray[jj-1]) / ID - 1) + (2 * (hFArray[jj-1]) / ID - 1)*pow(1 - pow(2 * (hFArray[jj-1]) / ID - 1, 2), 0.5));
					...
					Sum3 = Sum3 + (VTb / Lu)*(1 - HLTB)*(zArray[jj] - zArray[jj - 1]);
					SumNew = SumNew + ((zArray[jj] - zArray[jj - 1]) / 6.0)*(HLTBPast + 2 * y1 + 2 * y2 + HLTBNew);
				}
				...
				WLCal = (RhoL * (Ls)*Pi * ID*ID*0.25*HLLS + RhoL * Pi * ID*ID*0.25*HLTBAvg*Lf)*(VTb / Lu) - x;
			...
			fS = FannningFrictionFactor(RhoS, Vm, ID, MuL, PipeRoughness);// MuL :)))))))))
			Tsm = (2.0 / 2.5)*(0.5*fS*RhoS*Vm*Vm + 0.25*ID*RhoL*HLTBAvg*(VTb - VLTB)*(Vm - VLTB) / Ls);
   ...
   //the two values of negative and positive values shoudl be checked for each case
	double FilmProfile::LfDetermination(double InitialGuess,double dx, double Epsilon,double hMax, double EpsilonZh) {
		double Lf = 16, Eps = 10;;
		double F = 10;
		double err1 = 10;
		double check;
		...
		while (err > Epsilon)
		{
			FPrime = (hFDetermination(x + dx,hMax, EpsilonZh) - hFDetermination(x, hMax, EpsilonZh)) / dx;
	...
	void FilmProfile::HyrodynamicProperties(double InitialGuess, double dx, double Epsilon, double hMax, double EpsilonZh) {
		XSave.clear();
		HLTBSave.clear();
		std::ofstream output;
		output.open("Output.txt");
		...
		hFDetermination(LfFinal, hMax,EpsilonZh);
			...
			output << "Gas core velocity (Vg):	" << VTb - (VTb - VGLS)*(1 - HLLS) / (1 - HLTBAvg) << "	m/s" << std::endl;
			...
			for (int j = 0; j < ZSize + 1; j++) {
				HLTB = (1 / Pi)*(Pi - acos(2 * (hFArray[j]) / ID - 1) + (2 * (hFArray[j]) / ID - 1)*pow(1 - pow(2 * (hFArray[j]) / ID - 1, 2), 0.5));
